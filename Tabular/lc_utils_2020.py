...
def modify_loan_status(df, rate=0.10, probs=[0.65,0.35]) :
    df['tmp_ls'] = df["annual_inc"]*rate < df["avg_cur_bal"]
    # if condition above is true, just insert a True, otherwise keep previously existing loan status
    df['tmp_ls']  = df['tmp_ls'].where(df['tmp_ls'],df["loan_status"])
    # create a column of random charge/current
    vals = ['Current', 'Charged Off']
    df['r'] = np.random.choice(vals, len(df), p=probs)
...
def handle_employee_length(df) :
    ...
    df['emp_bin'] = np.where(df['emp_bin'] =='5 years',5,df['emp_bin'])
    df['emp_bin'] = np.where(df['emp_bin'] =='6 years',6,df['emp_bin'])
    df['emp_bin'] = np.where(df['emp_bin'] =='7 years',7,df['emp_bin'])
